...
object PedidosTable : Table("pedido") {
    ...
    val valorTotal = double("valor_total").nullable()
    val status = varchar("status", 50).nullable()
    val valorCustoTotal = double("valor_custo_total").nullable()
    val lucroBruto = double("lucro_bruto").nullable()
    val valorFrete = double("valor_frete").nullable()
    val tipoEnvio = varchar("tipo_envio", 50).nullable()
    val cepDestino = varchar("cep_destino", 10).nullable()
    val prazoEnvio = varchar("prazo_envio", 50).nullable()
    val comprimentoCm = double("comprimento_cm").nullable()
    val larguraCm = double("largura_cm").nullable()
    val alturaCm = double("altura_cm").nullable()
    val pesoKg = double("peso_kg").nullable()
    val formaPagamento = varchar("forma_pagamento", 50).nullable()
    val dataPagamento = varchar("data_pagamento", 50).nullable()
    val entregue = bool("entregue").nullable()
    override val primaryKey = PrimaryKey(id)
}
...
object PedidoItensTable : Table("pedido_item") {
    ...
    val tipo = varchar("tipo", 20).nullable()
    val nomeProduto = varchar("nome_produto", 255).nullable()
    val produtoNome = varchar("produto_nome", 255).nullable()
    val quantidade = integer("quantidade").nullable()
    val precoUnitario = double("preco_unitario").nullable()
    val valorTotal = double("valor_total").nullable()
    val custoUnitario = double("custo_unitario").nullable()
    override val primaryKey = PrimaryKey(id)
}
