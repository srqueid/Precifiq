...
object FornecedoresTable : Table("fornecedor") {
    ...
    val cnpjCpf = varchar("cnpj_cpf", 20).uniqueIndex()
...
object ProdutoVariacoesTable : Table("produto_variacao") {
    ...
    val tamanhoMedida = double("tamanho_medida")
    val unidadeMedidaTamanhoId = integer("unidade_medida_tamanho_id").references(UnidadesMedidaTable.id)
...
object PedidosCompraTable : Table("pedido_compra") {
    ...
    val valorTotalItens = double("valor_total_itens")
    val valorFrete = double("valor_frete")
    val valorFinalConfirmado = double("valor_final_confirmado")
    val formaPagamento = varchar("forma_pagamento", 20)
    override val primaryKey = PrimaryKey(id)
}
